...
function ProductController() {
  ...
  const fetchProducts = async () => {
    try {
      const response = await axios.get(
        "http://localhost:8080/admin/getProduct"
      );
  ...
  const handleAddOrUpdateProduct = async () => {
    ...
    try {
      if (isEditing) {
        await axios.put(
          `http://localhost:8080/admin/updateProduct/${editingProductId}`,
          product
        );
        setMessage("Product updated successfully!");
      } else {
        await axios.post("http://localhost:8080/admin/addProduct", product);
